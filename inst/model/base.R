...
raw_dat <- read.csv("ignore/RequestData_pre_ranked.csv")
...
championMapping <- read.csv("ignore/championMapping.csv", row.names = 1)
dat$champion <- left_join(dat, championMapping[,1:2], by= "champion")$"championNames"

champion_roles_df <- read.csv("ignore/champion_roles_df.csv")
...
highestWinProb_group <- function(dat, n){
  ...
  dat <- merge(dat_temp, championMapping[,c("championNames","roles_combined")], by.x = "champion", by.y = "championNames", all.x = TRUE)
